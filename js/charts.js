  ...
  data.segments.forEach((segment, i) => {
    data.tiers.forEach((tier, j) => {
      ...
      cell.append('text')
        ...
        .attr('dy', '0.35em')
        .attr('text-anchor', 'middle')
        // Use a light color so values are readable in dark mode as well
        .attr('fill', '#f9fafb')
        .attr('font-size', '14px')
        .attr('font-weight', 'bold')
 ...
 * Shows Revenue vs Customer Growth with scenario points
 *
 * @param {string} containerId - DOM element ID
 ...
 *
 * @param {string} containerId - DOM element ID
 * @param {Array} data - [{ name, customers_pct, revenue_pct, repeat_loss_pct, aov_pct }]
 */
export function renderComparisonBarChart(containerId, data, options = {}) {
  ...
  const sanitizedData = data.map(d => ({
    name: d.name || 'Unnamed',
    customers_pct: Number.isFinite(d.customers_pct) ? d.customers_pct : (Number.isFinite(d.customers_pct) ? d.customers_pct : 0),
    revenue_pct: Number.isFinite(d.revenue_pct) ? d.revenue_pct : 0,
    aov_pct: Number.isFinite(d.aov_pct) ? d.aov_pct : (Number.isFinite(d.aov_pct) ? d.aov_pct : 0),
    repeat_loss_pct: Number.isFinite(d.repeat_loss_pct) ? d.repeat_loss_pct : (Number.isFinite(d.repeat_loss_pct) ? d.repeat_loss_pct : 0)
  }));
 ...
 *
 * @param {string} containerId - DOM element ID
 * @param {Array} data - [{ name, dimensions: {revenue, growth, aov, churn, cltv} }]
 */
