 ...
 * @param {number} baseCustomers - Current customer count
 * @param {number} elasticity - Price elasticity coefficient
 * @returns {Object} Forecast object
 */
export function forecastDemand(currentPrice, newPrice, baseCustomers, elasticity) {
  if (!currentPrice || !newPrice || !baseCustomers || !elasticity) {
  ...
  const forecastedCustomers = baseCustomers * Math.pow(priceRatio, elasticity);

  // Calculate changes
  const change = forecastedCustomers - baseCustomers;
  const percentChange = (change / baseCustomers) * 100;

  return {
    baseCustomers,
    forecastedCustomers: Math.round(forecastedCustomers),
 ...
 * @param {Object} currentDistribution - { tier: customerCount }
...
/**
 * Calculate repeat-loss elasticity (how repeat loss rate changes with price)
 * @param {string} tier - Tier name
 * @param {number} priceChangePct - Price change percentage (e.g., 0.10 for 10% increase)
 * @param {number} baselineRepeatLoss - Current repeat loss rate
 * @returns {Promise<Object>} Forecast repeat loss
 */
export async function forecastChurn(tier, priceChangePct, baselineRepeatLoss) {
  const params = await loadElasticityParams();

  // Special handling for bundle tier
  if (tier === 'bundle') {
    // Bundles have lower repeat-loss sensitivity (better retention)
    const bundleRepeatLossElasticity = 0.3; // Lower than ad_free (0.5)
    const repeatLossChangePct = bundleRepeatLossElasticity * priceChangePct;
    const forecastedRepeatLoss = baselineRepeatLoss * (1 + repeatLossChangePct);

    return {
      baselineRepeatLoss,
      forecastedRepeatLoss,
      change: forecastedRepeatLoss - baselineRepeatLoss,
      changePercent: (repeatLossChangePct * 100),
      isBundle: true
    };
  }

  if (!params.repeat_loss_elasticity[tier]) {
    throw new Error(`Repeat-loss elasticity not available for tier: ${tier}`);
  }

  const repeatLossElasticity = params.repeat_loss_elasticity[tier].repeat_loss_elasticity;
  const repeatLossChangePct = repeatLossElasticity * priceChangePct;
...
/**
 * Calculate acquisition elasticity (how new customers change with price)
 * @param {string} tier - Tier name
 * @param {number} priceChangePct - Price change percentage
 * @param {number} baselineAcquisition - Current weekly new customers
